...
export async function PUT(req: Request) {
  ...
  try {      
    switch (plugin.source) {
      case "spigot":
        version = await downloadSpigot(plugin.name, plugin.version, plugin.location, output, server);
        ...
        break;
      case "github":
        version = await downloadGithub(plugin.name, plugin.version, plugin.location, output, server);
        success = !!version;
        break;
      case "bukkit":
